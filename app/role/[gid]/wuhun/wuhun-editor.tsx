...
import {
  coverStoreDataContentWhjpRequest,
  queryWuhunStoreInfoRequest,
} from '@/api-request';
import {
  CodeEditor,
  CodeEditorAction,
...
export const WuhunContentEditor = ({ gid }: { gid: string }) => {
  const { isPending, data, refetch } = useQuery({
    queryKey: ['wuhun-info', gid],
    queryFn: ({ queryKey }) => queryWuhunStoreInfoRequest(queryKey[1]),
  });

  const contentActionRef = useRef<CodeEditorAction>();

  const { mutate, isPending: isSubmiting } = useMutation({
    mutationFn: coverStoreDataContentWhjpRequest,
    onSuccess: () => {
      refetch();
      toast.success('保存成功');
